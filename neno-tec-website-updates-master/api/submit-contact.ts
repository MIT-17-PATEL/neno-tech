...
/* ── Sanitize: strip HTML, escape formula injection ──────────── */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const payload = new URLSearchParams({
    ...
    timeline: sanitize(body.timeline || '', 50),
    message: sanitize(body.message, 2000),
  });

  /* ── Forward to Google Apps Script (server-side, URL is hidden) ── */
  const endpoint = process.env.SHEET_ENDPOINT;
  if (!endpoint) {
    console.error('SHEET_ENDPOINT not configured');
  ...
  try {
    ...
    // Google Apps Script returns 302 on successful doPost — data is saved before the redirect
